...
|Método|   Rota   |                   Descrição                   |
 —————————————————————————————————————————————————————————————————
|GET   |/notas    |Retorna todas as notas lançadas.               |
 —————————————————————————————————————————————————————————————————
|POST  |/notas    |Lança uma nota (valida se o alunoId existe).   |
 —————————————————————————————————————————————————————————————————
|PUT   |/notas/:id|Atualiza as informações de uma nota específica.|
 —————————————————————————————————————————————————————————————————
|DELETE|/notas/:id|Remove uma nota do sistema.                    |
 —————————————————————————————————————————————————————————————————
...
- Persistência de Dados: Todas as informações são salvas nos arquivos alunos.json e notas.json.

- Integridade: Não é possível lançar uma nota para um aluno que não esteja cadastrado, não é possível lançar uma nota negativa ou maior que 10 e não é possível cadastrar um aluno menor de 14 anos e maior que 120 anos.

- Feedback Visual: O front-end exibe mensagens de sucesso ou erro para todas as operações.

- Design: Interface estilizada com cores cianas e brancas, organizada em seções.